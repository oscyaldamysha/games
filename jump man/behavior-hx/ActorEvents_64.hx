...
class ActorEvents_64 extends ActorScript
{
	...
	override public function init()
	{
		...
		addWhenUpdatedListener(null, function(elapsedTime:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				if(((actor.getAnimation() == "walking right") && isKeyPressed("action1")))
				{
					createRecycledActor(getActorType(66), Engine.engine.getGameAttribute("x of hero"), Engine.engine.getGameAttribute("y of hero"), Script.MIDDLE);
					getLastCreatedActor().setAnimation("" + "right");
					getLastCreatedActor().applyImpulseInDirection(10, 10);
				}
			}
		});
