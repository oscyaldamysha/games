...
class ActorEvents_56 extends ActorScript
{
	...
	override public function init()
	{
		...
		addWhenUpdatedListener(null, function(elapsedTime:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				Engine.engine.setGameAttribute("debug log", actor.getAnimation());
				if((Engine.engine.getGameAttribute("up is down") == false))
				{
					if(isKeyDown("right"))
					{
				...
				else if((Engine.engine.getGameAttribute("up is down") == true))
				{
					actor.setAnimation("" + "jumping");
				}
			}
		...
		addWhenUpdatedListener(null, function(elapsedTime:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				if(isKeyDown("up"))
				{
					Engine.engine.setGameAttribute("up is down", true);
				}
				else
				{
					Engine.engine.setGameAttribute("up is down", false);
				}
			}
		});
